...
#####################################################################
# Run DNN
model = H2ODeepLearningEstimator(model_id='inertia_first_try'
...
load_dnn=h2o.load_model('models/dnn_inertmed/inertia_first_try')
perf=load_dnn.model_performance(test_data=test)
perf
# Get model predictions on test data, put directly in pandas DataFrames inside dictionary
pred_dnn = model.predict(test_data=test)
predict['predict_dnn']=pred_dnn.as_data_frame()['predict'].values
...
data = [inertia,prediction_aml,prediction_dnn,error_aml,error_dnn]
